...
/* A fast, yet precise, exp approximation */
const int exp_precision = 10;
double expTable[1<<exp_precision];
void expFastInit(void) {
  for(int i = 0; i < (1<<exp_precision); i++)
    expTable[i] = pow(2, 1.*i/(1<<exp_precision));
}

inline double exp_approx(double x) {
  x *= 1.44269504088896340737*(1<<exp_precision);
  if(x >= 1024*(1<<exp_precision))
    return INFINITY;
  if(x < -1074*(1<<exp_precision))
    return 0;
  int xQuant = lrint(x);
  double xQuantD = xQuant;
  long long exp = (xQuant >> exp_precision)+1023;
  union { double dbl; long long ll; } conv;
  conv.ll = exp <= 0 ? 1LL<<(exp+51) : exp<<52;
  return conv.dbl * expTable[xQuant & ((1<<exp_precision)-1)] * ((x-xQuantD)*(1/(1.44269504088896340737*(1<<exp_precision)))+1);
}
...
struct toxicity_parameters {
  ...
  template<typename U>
  U proba_tox(const U& dose_tox) {
    U e = exp_approx(beta0 + beta1*dose_tox);
    U p =  e/(1+e);
    return p*(1-2e-10)+1e-10;
  }
};
...
struct efficacy_parameters {
  ...
  efficacy_parameters(double gamma0, double gamma1, int tau, const vector<double>& doseE):
    gamma0(gamma0), gamma1(gamma1), tau(tau), doseE(vec(doseE.data(), doseE.size())) {}
  efficacy_parameters(const vector<double>& doseE):
    gamma0(0), gamma1(0), tau(0), doseE(vec(doseE.data(), doseE.size())) {}

  vec responseRate(const uvec& id_dose) const {
    unsigned tau = min((int)doseE.size()-1, max(0, this->tau));
    uvec sup_tau = id_dose > tau;
    vec e = exp_approx(gamma0 + gamma1*doseE.elem(sup_tau*tau+(1-sup_tau)%id_dose));
    vec p =  e/(1+e);
    return p*(1-2e-10)+1e-10;
  }

  double responseRate(unsigned id_dose) const {
    unsigned tau = min((int)doseE.size()-1, max(0, this->tau));
    unsigned sup_tau = id_dose > tau;
    double e = exp_approx(gamma0 + gamma1*doseE(sup_tau*tau+(1-sup_tau)*id_dose));
    double p =  e/(1+e);
    return p*(1-2e-10)+1e-10;
  }
};
...
struct estimations {
  ...
  estimations(const vector<double>& doseE):
    eff_params(doseE),
    pi(doseE.size()), ptox_inf(doseE.size()), resp(doseE.size()),
    qeff_inf(doseE.size()), resp2(doseE.size()), proba_tau(doseE.size())
  { }
};
...
struct trial_data;
  ...
  typedef estimations (*estimator)(const trial_data& trial_data, int group, bool final);
...
struct results {
  ...
  void accumulate(const trial_data& trial_data) {
    for(int pat=0; pat<trial_data.pat_incl; pat++){
      int dose_pat = trial_data.id_dose[pat];
...
bool wait_patient(trial_data& trial_data, const true_data& true_data, int& group) {
  ...
  if(rate == 0) {
    ...
    if(!HAS_TIME)
      ...
      double time_incl = exp_rng(r, rate);
...
void include_patient(trial_data& trial_data, const true_data& true_data, int group) {
  ...
  trial_data.id_dose.push_back(trial_data.cdose[group]);
  ...
  trial_data.toxicity.push_back(uni_rng(r) < pi_ij);
  ...
  if(HAS_TIME) {
    ...
    trial_data.time_eff.push_back(exp_rng(r, param_exp));

    trial_data.time_incl.push_back(trial_data.time_cur);
  } else
    trial_data.efficacy.push_back(uni_rng(r) < resp_ij);
...
estimations estimate_ra(const trial_data& trial_data, int group, bool final) {
  ...
  const vec dose_tox =
    ...
    .elem(uvec(trial_data.id_dose.data(), NPatientsCycle));

  uvec in_group = find(ivec(trial_data.group.data(), NPatientsCycle) == group);
  vec efficacy, time_min_eff, weights;
  if(HAS_TIME) {
    efficacy = vec(in_group.size());
    time_min_eff = vec(in_group.size());
  ...
  const uvec id_dose_eff = uvec(trial_data.id_dose.data(), NPatientsCycle).elem(in_group);

  /* MCMC initalization and sampling */
  efficacy_parameters eff_params(0, 1, ndose-1, doseE);
  toxicity_parameters tox_params(0, 1);

  vec pi_jk, resp_jk;

  std::function<void ()> model = [&]() {
    if(HAS_TIME)
      resp_jk = weights%eff_params.responseRate(id_dose_eff);
    else
  ...
  while(itbeta0 != itbeta0end) {
    params_draws_list.push_back(make_pair(
       efficacy_parameters(*itgamma0, *itgamma1, *ittau, doseE),
  ...
  for(int d = 0; d < ndose; d++) {
    ...
    for(auto draw: params_draws_list){
      double proba_tox = draw.second.proba_tox(trial_data.doseT[d]);
  ...
  for(int d = 0; d < ndose; d++) {
    ...
    for(auto draw: params_draws_list){
      double resp_rate = draw.first.responseRate(d);
...
/* Estimate toxicity and efficacy parameters.
   The efficacy is estimated only for the given group. */
estimations estimate_pm(const trial_data& trial_data, int group, bool) {
  const int ndose = trial_data.doseT.size();
...
int find_next_dose(trial_data& trial_data, int group, double c_tox, double c_eff,
  ...
  if(trial_data.startup_end[group] == -1) {
    ...
    for(int pat = 0; pat < trial_data.pat_incl; pat++)
      if(trial_data.group[pat] == group &&
         trial_data.id_dose[pat] == trial_data.cdose[group] &&
  ...
  estimations estim = trial_data.est(trial_data, group, final);
  if(estimRet != NULL)
    *estimRet = estim;
  ...
  for(int pat = 0; pat < trial_data.pat_incl; pat++){
    if(trial_data.group[pat] == group){
      pat_explo_dose[trial_data.id_dose[pat]]++;
      if((int)trial_data.id_dose[pat] > highest_tested){
        highest_tested = trial_data.id_dose[pat];
      }
    }
  }
  ...
  if(final)
    max_candidate_dose = highest_tested;
  else
    max_candidate_dose = max(min(ndose-1, cdose+1), highest_tested);
...
static void dfmta_next(int* has_time, int* ra_pm /* true for RA, false for PM */,
                       ...
                       int* startup_end /* -1 if startup has not ended */,
                       double* time_cur,
                       int* pat_incl_group,
                       int* id_dose, int* group,
  ...
  trial_data trial_data(est, doseTV, doseEV, *ngroups, *s_2, _s_1);
  ...
  trial_data.startup_end[group_cur] = *startup_end;
  trial_data.time_cur = *time_cur;
  trial_data.pat_incl_group = vector<int>(pat_incl_group, pat_incl_group+*ngroups);
  trial_data.id_dose = vector<unsigned int>(id_dose, id_dose+pat_incl);
  ...
  vector<double> dummy;
  estimations estim(dummy);
  *cdose = find_next_dose(trial_data, group_cur, *c_tox, *c_eff, *final, &estim);
  *startup_end = trial_data.startup_end[group_cur];

  if(startup_end[group_cur] != -1)
    for(int d = 0; d < *ndose; d++) {
      pi[d] = estim.pi[d];
...
static R_NativePrimitiveArgType dfmta_simu_args[] =
  {LGLSXP, LGLSXP,
   ...
   INTSXP, INTSXP, INTSXP,
   REALSXP};
static const int dfmta_simu_nargs = 32;
...
static void dfmta_simu(int* has_time, int* ra_pm /* true for RA, false for PM */,
  ...
  for(int trial=0; trial<*ntrial; trial++){
    estimator est = *ra_pm ? estimate_ra : estimate_pm;
    struct trial_data trial_data(est, doseTV, doseEV, *ngroups, *s_2, _s_1);
    ...
    while(true) {
      ...
      if(trial_data.cdose[group] != -1) {
        include_patient(trial_data, true_data, group);
        if(trial_data.pat_incl_group[group] > *npatientspergroup)
          throw std::logic_error("Internal error: too many patients in a group.");
        if(trial_data.pat_incl_group[group] == *npatientspergroup)
          trial_data.cdose[group] = -1; /* No more patiens for this group */
      }
    }

    for(int group = 0; group < *ngroups; group++){
      int recom = find_next_dose(trial_data, group, *c_tox, *c_eff, true);
      if(recom == -1) {
        results.inconc[group]++;
      }
      else {
        results.rec_dose[recom][group]++;
        for(int pat = 0; pat < trial_data.pat_incl; pat++){
          if(trial_data.group[pat] == group &&
             trial_data.id_dose[pat] == recom) {
            results.n_pat_mtd[group]++;
          }
        }
      }
    }
    results.accumulate(trial_data);
  ...
  *duration  = results.duration;
  ...
  if(results.nb_trials != *ntrial)
    throw std::logic_error("Internal error: invalid ntrials.");
