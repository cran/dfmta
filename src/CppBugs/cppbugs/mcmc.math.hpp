...
namespace cppbugs {
  ...
  double uniform_logp(const T& x, const U& lower, const V& upper) {
    return arma::any((x < lower) + (x > upper)) ? -std::numeric_limits<double>::infinity() : -arma::accu(log_approx(upper - lower));
  }
  ...
  double beta_logp(const T& x, const U& alpha, const V& beta) {
    return arma::any((x <= 0 ) + (x >= 1)) ||
      arma::any(alpha <= 0) || arma::any(beta <= 0) ?
      -std::numeric_limits<double>::infinity() :
      arma::accu(lgamma(alpha+beta) - lgamma(alpha) - lgamma(beta) + arma::schur(alpha - 1.0, log_approx(x)) + arma::schur(beta - 1.0, log_approx(1.0 - x)));
  }

  template<typename T, typename U, typename V>
  double binom_logp(const T& x, const U& n, const V& p) {
    if(arma::any((p <= 0) + (p >= 1)) || arma::any((x < 0) + (x > n))) {
      return -std::numeric_limits<double>::infinity();
    }
  ...
  double bernoulli_logp(const T& x, const U& p) {
    if( arma::any((p <= 0) + (p >= 1)) || arma::any((x < 0) + (x > 1))) {
      return -std::numeric_limits<double>::infinity();
    } else {
